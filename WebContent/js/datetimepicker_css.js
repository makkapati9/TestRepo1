...
function RenderCssCal(bNewCal)
{
	...
	{
	    vCalHeader += "<td align='center'><select name='MonthSelector' onChange='javascript:Cal.SwitchMth(this.selectedIndex);RenderCss();'>";
		for (i = 0; i < 12; i += 1)
		{
			if (i === Cal.Month)
			{
	...
	funcCalback += "var current_date = new Date();	var current_year = current_date.getFullYear(); var CalId = document.getElementById(id);if (datum=== 'undefined') { var d = new Date(); datum = d.getDate() + '/' +(d.getMonth()+1) + '/' + d.getFullYear(); } window.calDatum=datum;CalId.value=datum; var year = datum.split('-')[2]; var age = current_year - year;document.getElementById('age').value = age;  ";
...
function NewCssCal(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pShowSeconds, pEnableDateMode)
{
	...
	{
		Cal.Format = "DDMMYYYY"; // set date in DDMMYYYY formate
	}
	...
	exDateTime = document.getElementById(pCtrl).value; //getting Existing Date Time value in textbox.
