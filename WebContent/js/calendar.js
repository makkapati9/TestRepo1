...
function validatedate()
{
	 ...
	 var x=document.getElementById('orderDate').value;
     var date = new Date();

    if(x!="")
  	  {   	
    	var   fromyearfield = x.split("-")[0];
    	var   frommonthfield = x.split("-")[1];
    	var   fromdayfield = x.split("-")[2];
    	...
    	var   todayFullYear=date.getFullYear();
     
    	 if(fromyearfield>todayFullYear)
     	{
 		var SbarId = document.getElementById("msorderDate");

 		SbarId.style.display = "inline";
 		document.getElementById("msorderDate").innerHTML = "आदेश दिनांक की तारीख आज की तारीख से कम  या आज की तारीख होना चाहिए";
 		document.getElementById("msorderDate").scrollIntoView();
 		return false;
     	}
         
      if(frommonthfield>=todaymonth && fromyearfield>todayFullYear)
     	 {
      ...
      if(fromdayfield>todayDate && frommonthfield>=todaymonth && fromyearfield>=todayFullYear)
     	 {
  		var SbarId = document.getElementById("msorderDate");
...
function _fnGetTitleRow(iIndex)
{
	...
	oRowLayout.push("<table cellspacing='1' cellpadding='0' border='0' class='calendarTable_' onMouseDown=\"_fnMoveDivAbove('"+iIndex+"')\" style='height:57%; width:14%'>");
	oRowLayout.push("<tr>");
...
function _fnGetYearRow(iIndex)
{
	...
	oRowLayout.push("<tr>");
	oRowLayout.push("<td align='center' valign='middle' colspan='"+oCalendarArray['Day'].length+"' class='yearHeadingRow_' cellspacing='0' height='10%'>");

	oRowLayout.push("<table cellspacing='0' cellpadding='2' border='0' width='100%' class='yearHeadingRow_'>");
	oRowLayout.push("<tr>");
...
function _fnAssignDate(iIndex, sDay)
{
	...
	var sDateString1=$("#orderDate1").val();
	try
	{
		
		sDateString1 = fnSetDateFormat(oDateFormat);
		if(!sDateString1)
			sDateString1 = oDateFormat['Date'] +"/"+ oDateFormat['Month'] +"/"+ oDateFormat['FullYear'];
	}
	catch(e)
	{
		sDateString1 = oDateFormat['Date'] +"/"+ oDateFormat['Month'] +"/"+ oDateFormat['FullYear'];
	}
	var sDateString2=$("#loanDate").val();
	try
	{
		
		sDateString2 = fnSetDateFormat(oDateFormat);
		if(!sDateString1)
			sDateString2 = oDateFormat['Date'] +"/"+ oDateFormat['Month'] +"/"+ oDateFormat['FullYear'];
	}
	catch(e)
	{
		sDateString2 = oDateFormat['Date'] +"/"+ oDateFormat['Month'] +"/"+ oDateFormat['FullYear'];
	...
	var pageView = $("#ssnPgVal").val();
				...
				var bandhakArea = $("#bandhakArea").val();
				...
				if(bankAddress !="" ){
					var rmrk ="";
					rmrk = "राशि " +loanAmount+" रूपये   मे यह  भूमि का "+ bandhakArea+ " क्षेत्रफल   " +bankName+" शाखा  "+ bankAddress+ " मे दिनांक " +sDateString2+  " से  बंधक है| ";
					//alert(rmrk);
					$('#remark').val(rmrk);
			      }
				if(orderNo !="" && orderBy != ""){
						var rmrk ="";
						rmrk = "(मिटटी वर्गीकरण) "+orderBy+" दवारा प्रकरण क्रमांक "+orderNo+" दिनांक "+sDateString+" को की गई |";
						$('#orderRemarks').val(rmrk);
				}
				if(orderNo1 !="" && orderBy1 != ""){
					
					    var rmrk1 ="";
						var sinAsin = $("#chkSncht").val();
						if(sinAsin == "1"){
							rmrk1 = "(यह ज़मीन असिंचित है )"+orderBy1+" दवारा प्रकरण क्रमांक "+orderNo1+" दिनांक "+orderDate1+" को की गई |";

						}
						else{
							rmrk1 = "(यह ज़मीन सिंचित है )"+orderBy1+" दवारा प्रकरण क्रमांक "+orderNo1+" दिनांक "+sDateString1+" को की गई |";

						}
											
						$('#orderRemarks1').val(rmrk1);
				}
	...
	oCalendarArray['Input'].value = sDateString1;
