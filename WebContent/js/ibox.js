/**
 * iBox 2.2 (Build 1612) For more info & download:
 * http://www.ibegin.com/labs/ibox/ Created as a part of the iBegin Labs Project -
 * http://www.ibegin.com/labs/ For licensing please see readme.html (MIT Open
 * Source License)
 */
var iBox = function() {
	var _pub = {
		...
		is_firefox : navigator.appName == "Netscape"
		 ...
		 * 
		 * @param {String}
		 *            path Relative or absolute path to this file.
		 */
		...
		checkTags : function(container, tag_name) {
			...
			for ( var i = 0; i < els.length; i++) {
				if (els[i].getAttribute(_pub.attribute_name)) {
					...
					if ((t.indexOf("ibox") != -1) || t.toLowerCase() == "ibox") { // check
																					// if
																					// this
																					// element
																					// is
		...
		html : function(content, params) {
			...
			// reset offsets
			offset.container = [ els.wrapper.offsetLeft * 2,
					els.wrapper.offsetTop * 2 ];
			offset.wrapper = [
					els.wrapper.offsetWidth - els.content.offsetWidth,
					els.wrapper.offsetHeight - els.content.offsetHeight ];
			...
			// XXX: Fix for inline containers which had elements that were
			// hidden
		...
		hide : function() {
			...
			_pub.fade(els.overlay, _pub.getOpacity(null, els.overlay), 0,
					_pub.fade_out_speed, function() {
		...
		reposition : function() {
			...
			if (active.dimensions) {
				...
				if (height.toString().indexOf('%') != -1) {
					els.wrapper.style.height = (Math.max(
							document.documentElement.clientHeight,
							document.body.clientHeight, pagesize.height) - offset.container[0])
							* (parseInt(height) / 100) + 'px';
				} else if (height) {
					els.content.style.height = height + 'px';
					// TODO: if we dont set wrapper height, it doesnt restrict
					// the height and the box is fine
					// so offset.wrapper[1] must not be correct
					els.wrapper.style.height = els.content.offsetHeight
							+ offset.wrapper[1] + 'px';
				} else {
					els.wrapper.style.height = els.content.offsetHeight
							+ offset.wrapper[1] + 'px';
				...
				if (width.toString().indexOf('%') != -1) {
					els.wrapper.style.width = (Math.max(
							document.documentElement.clientWidth,
							document.body.clientWidth, pagesize.width) - offset.container[1])
							* (parseInt(width) / 100) + 'px';
					var container_offset = 0;
				} else {
					els.content.style.width = width + 'px';
					els.wrapper.style.width = els.content.offsetWidth
							+ offset.wrapper[0] + 'px';
				}
				...
				// TODO: this isn't adjusting to the right height for containers
				// that are smaller than the page height
			...
			els.overlay.style.height = Math.max(document.body.clientHeight,
					document.documentElement.clientHeight)
					+ 'px';
		},
		...
		center : function(obj) {
			...
			var x = Math.round((pageSize.width - emSize.width) / 2
					+ scrollPos.scrollX);
			var y = Math.round((pageSize.height - emSize.height) / 2
					+ scrollPos.scrollY);
			if (obj.offsetLeft)
				x -= obj.offsetLeft;
			if (obj.offsetTop)
		...
		getScrollPos : function() {
			...
			return {
				scrollX : document.body.scrollLeft || window.pageXOffset
						|| (docElem && docElem.scrollLeft),
				scrollY : document.body.scrollTop || window.pageYOffset
		...
		fade : function(obj, start, end, speed, callback) {
			...
			var increment = Math.ceil(opacity_difference / steps); // how much
																	// to incr
																	// per step

			obj.fader = setInterval(_pub.bind(function(e, obj, increment, end,
					callback) {
				var opacity = _pub.getOpacity(e, obj) + increment;
				_pub.setOpacity(e, obj, opacity);
				if ((increment < 0 && opacity <= end)
						|| (increment > 0 && opacity >= end)) {
		...
		/**
		 * Causes all event listeners attached to `name` event to execute.
		 * 
		 * @param {String}
		 *            name Event name
		 */
		...
		handleTag : function(e) {
			...
			else if (this.target && !params.ignore_target)
				var url = this.target;
			else
				var url = this.href;
	...
	var create = function(elem) {
		...
		if (!_pub.is_firefox)
			...
			els.overlay.style.backgroundImage = "url('" + _pub.base_url
					+ "images/bg.png')";
	 ...
	 * 
	 * @param {String}
	 *            tag The name of the tag (e.g. 'a')
	 * @param {HTMLObject}
	 *            container The container to restore tags within (defaults to
	 *            document)
	 */
	var hideTags = function(tag, container) {
		...
		for ( var i = 0; i < list.length; i++) {
			if (_pub.getStyle(list[i], 'visibility') != 'hidden'
	...
	var showInit = function(title, params, callback) {
		...
		if (!_pub.is_firefox)
			...
			var amount = 100;
		_pub.fade(els.overlay, _pub.getOpacity(null, els.overlay), amount,
				_pub.fade_in_speed, callback);
	...
	var drawCSS = function() {
		...
		htmDiv.innerHTML = '<p>x</p><style type="text/css">' 
				+ '</style>';
		head.insertBefore(htmDiv.childNodes[1], head.firstChild);

		htmDiv.innerHTML = '<p>x</p><style type="text/css">' + core_styles
				+ '</style>';
		head.insertBefore(htmDiv.childNodes[1], head.firstChild);
	};
	...
	/**
	 * Handles embedded containers in the page based on url of #container. This
	 * _ONLY_ works with hidden containers.
	 */
			...
			render : function(url, params) {
				...
				if (!elemSrc) {
					...
					_pub
							.html(
									document
											.createTextNode('There was an error loading the document.'),
									params);
				} else {
	...
	var iBoxPlugin_YouTube = function() {
		...
		return {
			...
			render : function(url, params) {
				...
				var html = '<span><object width="100%" height="100%" style="overflow: hidden; display: block;"><param name="movie" value="http://www.'
						+ domain
						+ '/v/'
						+ id
						+ '"/><param name="wmode" value="transparent"/><embed src="http://www.'
						+ domain
						+ '/v/'
						+ id
						+ '" type="application/x-shockwave-flash" wmode="transparent" width="100%" height="100%"></embed></object></span>';
	...
	var iBoxPlugin_Document = function() {
		return {
			...
			render : function(url, params) {
				...
				_pub.http.setRequestHeader("Content-Type",
